...
<p align="center">
  <a href="#category--where-dynamic-harness-sits"><img src="https://img.shields.io/badge/Layer-L3%20Meta--Factory-orange" alt="Layer"></a>
  <a href="#category--where-dynamic-harness-sits"><img src="https://img.shields.io/badge/Sub--layer-Team--Architecture%20Factory-teal" alt="Sub-layer"></a>
...
> **Dynamic-Harness is a fork of [Harness](https://github.com/revfactory/harness) that applies Claude Code's dynamic Workflow tool.** It keeps everything Harness does — turning a one-line domain description into an agent team and the skills they use, across six team-architecture patterns — and adds a **4th execution mode**: deterministic JavaScript orchestration (`agent()` / `pipeline()` / `parallel()`) where the *script* holds the loop, branching, and intermediate results.
>
> Say **"build a harness for this project"** (English) / **"하네스 구성해줘"** (한국어) / **"ハーネスを構成して"** (日本語) — same trigger, now with dynamic workflows on top.

## Overview

Dynamic-Harness leverages Claude Code's agent team system to decompose complex tasks into coordinated teams of specialized agents, and adds a **Workflow execution mode** for large-scale, deterministic, resumable orchestration. Say "build a harness for this project" and it automatically generates agent definitions (`.claude/agents/`) and skills (`.claude/skills/`) tailored to your domain. The base team-architecture-factory capability comes from upstream [Harness](https://github.com/revfactory/harness); Dynamic-Harness extends it with the dynamic Workflow tool (the 4th execution mode).

## Category — Where Dynamic-Harness Sits

Dynamic-Harness lives at the **L3 Meta-Factory** layer of the Claude Code ecosystem — the layer that generates other harnesses rather than being one. Inside L3, we pick a specific sub-layer: **Team-Architecture Factory**.
...
> Archon generates deterministic runtime configurations. Dynamic-Harness generates team architectures (pipeline, fan-out/fan-in, expert pool, producer-reviewer, supervisor, hierarchical delegation) plus the skills agents use — and can orchestrate them as deterministic Workflow scripts. Different sub-layers of the same L3. Pick Archon for runtime determinism, Dynamic-Harness for team architecture, or combine them.
...
Files generated by Dynamic-Harness:
...
Copy any prompt below into Claude Code after installing Dynamic-Harness:
...
## Coexistence — Dynamic-Harness and Neighbors

Dynamic-Harness is not alone in the Claude Code / agent-framework ecosystem. The following repos live in adjacent layers; each is described in a parallel "X is …, Dynamic-Harness is …" form so you can pick the one that fits your need or combine several. (The same positioning holds for upstream Harness, which Dynamic-Harness builds on.)

| Repo | Their position | Relationship to Dynamic-Harness |
|------|----------------|---------------------------------|
| [coleam00/Archon](https://github.com/coleam00/Archon) | "harness builder" — deterministic, repeatable runtime configurations | **Same L3, neighbor sub-layer.** Archon is a Runtime-Configuration Factory, Dynamic-Harness is a Team-Architecture Factory. Pick Archon for runtime determinism, Dynamic-Harness for team architecture, or combine them. |
| [SaehwanPark/meta-harness](https://github.com/SaehwanPark/meta-harness) | Codex port of the same concept | **Same L3, different runtime.** Use Dynamic-Harness on Claude Code, meta-harness on Codex. |
| [affaan-m/ECC](https://github.com/affaan-m/everything-claude-code) | "Agent harness performance & workflow layer" (sits on top of existing harnesses) | **Different layer.** ECC is a standardization layer across harnesses; Dynamic-Harness is a factory that generates harnesses. Serial combination possible. |
| [wshobson/agents](https://github.com/wshobson/agents) | Subagent / skill catalog (182 agents, 149 skills) | **Factory ↔ parts supply.** wshobson is a catalog to shop from; Dynamic-Harness designs the team. Absorb wshobson entries as parts inside a Dynamic-Harness-generated team. |
| [LangGraph](https://langchain-ai.github.io/langgraph/) | State-graph orchestration, LLM-agnostic | **Converging track.** LangGraph is for long-running, state-recoverable orchestration; Dynamic-Harness is for fast Claude-Code-native team design — and its Workflow mode adds deterministic, script-driven orchestration that narrows the gap. |

## Built with Harness (upstream foundation)

> These showcase projects and the research results below were produced with **upstream [Harness](https://github.com/revfactory/harness)**. Dynamic-Harness is a fork that inherits this foundation and adds the dynamic Workflow execution mode — the +60% A/B was measured on upstream Harness, **not** on Dynamic-Harness specifically.

### Harness 100

**[revfactory/harness-100](https://github.com/revfactory/harness-100)** — 100 production-ready agent team harnesses across 10 domains, available in both English and Korean (200 packages total). Each harness ships with 4-5 specialist agents, an orchestrator skill, and domain-specific skills — all generated by upstream Harness. 1,808 markdown files covering content creation, software development, data/AI, business strategy, education, legal, health, and more.
...
**A.** Archon generates deterministic runtime configurations — it's a **Runtime-Configuration Factory**. Dynamic-Harness generates agent team architectures (team structure, message protocols, review gates) — it's a **Team-Architecture Factory** — and can orchestrate them as deterministic Workflow scripts. They are **neighbor sub-layers of the same L3 Meta-Factory** and serve different needs. Pick Archon for runtime determinism, Dynamic-Harness for team-architecture patterns, or combine them (design architecture with Dynamic-Harness → deploy runtime with Archon).
...
- Sub-layer declaration: see the **Category — Where Dynamic-Harness Sits** section above
...
**A.** Currently the official runtime is Claude Code only. A Codex port of the same concept — [SaehwanPark/meta-harness](https://github.com/SaehwanPark/meta-harness) — is already public, so Codex teams can start there. Dynamic-Harness, like upstream Harness, chose "Claude-Code-native, deep" over "multi-runtime, shallow" (the Workflow execution mode is a Claude-Code-native feature); cross-runtime collaboration with sibling repos (meta-harness, harness-init, OpenRig) is on the roadmap.
