...
조사 각도가 4개를 크게 넘거나(수십 소스), 같은 리서치를 **반복 실행**한다면 워크플로우 모드가 적합하다. 번들된 `/deep-research`가 바로 이 팬아웃/팬인 패턴이다.

```js
const found = (await parallel(ANGLES.map(a => () =>
  agent(`${a} 각도로 조사하고 _workspace/02_${a}_raw.md에 기록`, { schema: FINDINGS_SCHEMA }))
)).filter(Boolean)
const report = await agent(`종합:\n${found.map(f => f.gist).join('\n')}`, { label: 'synthesizer' })
```

팀 모드와의 차이: 팀은 조사자 간 실시간 `SendMessage` 토론이 강점(소수·상호참조), 워크플로우는 **대규모 팬아웃·반복·결정적 통합**이 강점. 상세는 `references/workflow-mode.md`.
...
"최대 2회 재시도 후 강제 PASS", "50% REDO면 사용자 확인" 같은 **결정적 재시도 정책**은 모델 주도 루프보다 스크립트가 정확히 강제한다 — 카운터·상한·강제 PASS가 코드에 있어 무한 루프·정책 이탈이 구조적으로 불가능.

```js
const reviewed = await parallel(panels.map(p => () => (async () => {
  let img = await agent(`패널 ${p} 생성`, { label: `gen:${p}` })
  for (let tries = 0; tries < 2; tries++) {               // 상한을 JS가 보유
    const v = await agent(`패널 ${p} 검수: PASS/FIX/REDO`, { label: `review:${p}`, schema: VERDICT })
    if (v.verdict === 'PASS') return { p, status: 'PASS' }
    img = await agent(`패널 ${p} 재생성: ${v.fix}`, { label: `redo:${p}` })
  }
  return { p, status: 'FORCE_PASS' }                        // 2회 후 강제 PASS
})()))
const redoRate = reviewed.filter(r => r.status === 'FORCE_PASS').length / reviewed.length  // 50% 게이트
```

상세는 `references/workflow-mode.md`.
...
### 워크플로우 모드 변형 (대규모 시 권장)

파일이 수십~수백 개면 팀 모드의 "세션당 1팀" 제약에 막힌다. 감독자의 동적 배치 할당은 워크플로우의 **동시성 캡(16) + 파일 단위 `pipeline`**으로 더 자연스럽게 표현된다.

```js
// 정찰 먼저: 대상 파일 목록을 인라인으로 발견 (hybrid scout-then-fan-out)
const files = await discoverTargets()
const results = await pipeline(files,
  f => agent(`${f} 마이그레이션`, { label: `mig:${f}`, isolation: 'worktree' }),   // 병렬 수정 → worktree 격리
  (r, f) => agent(`${f} 검증`, { label: `verify:${f}`, schema: VERDICT }))
```

`isolation: 'worktree'`로 파일 병렬 수정 충돌을 방지하고, `pipeline`은 배리어가 없어 **파일 A 검증 중 파일 B 마이그레이션**이 진행된다. 16개씩 자동 큐잉되어 수백 개도 캡 안에서 처리. 이것이 팀 모드 대비 워크플로우의 가장 뚜렷한 우위 사례다. 상세는 `references/workflow-mode.md`.
