...
**소규모 협업의 최우선 기본값은 에이전트 팀이다.** 2개 이상의 에이전트가 통신하며 협업할 때는 반드시 에이전트 팀을 먼저 검토한다. 팀원 간 직접 통신(SendMessage)과 공유 작업 목록(TaskCreate)으로 자체 조율하며, 발견 공유·상충 토론·누락 보완이 결과 품질을 높인다. **단, 수십 개 이상으로 팬아웃하거나 결정적 시퀀스·반복 실행·재개가 필요한 대규모 작업은 워크플로우 모드를 우선 검토한다** — 팀은 "세션당 1팀·토큰 선형 증가" 제약 때문에 대규모 팬아웃에 부적합하다.
...
| **워크플로우** (대규모/결정적) | 수십~수백 에이전트 팬아웃, 결정적 시퀀스·품질 게이트, 반복 실행, 같은 세션 내 재개가 필요할 때 | JS 스크립트가 제어 흐름(루프·분기·중간결과)을 보유. `agent()`/`pipeline()`/`parallel()`. **research preview** |

**의사결정 순서:**
1. **규모·반복성 먼저 확인** — 수십 개 이상 팬아웃하거나, 오케스트레이션 자체가 반복 실행/재개돼야 하면 **워크플로우** 모드 (16 동시 / 1,000 총 에이전트 캡). Workflow 도구 불가 환경이면 팀/하이브리드로 폴백
2. 그 외 2명 이상 협업이면 에이전트 팀으로 설계 가능한지 검토 — 통신·실시간 피드백이 품질을 좌우하면 기본값
3. 팀 통신이 구조적으로 불필요하고(결과 전달만), 팀 오버헤드가 이득보다 클 때만 서브 에이전트 선택
...
> 워크플로우 모드(프리미티브, 6패턴 매핑, Template D, 비용·재개)는 `references/workflow-mode.md` 참조.
...
**모델 설정:** 기본값은 `model: "opus"`다 — 하네스 품질은 에이전트의 추론 능력에 직결되므로, 추론·생성·검증의 핵심 단계는 opus를 쓴다. **단, 모든 단계에 opus를 강제하지 않는다.** 단순·기계적 단계(형식 변환, 목록 수집, 정형 추출)는 하위 티어로 라우팅해 비용을 줄일 수 있다 — 멀티 에이전트는 chat 대비 토큰 비용이 크게 증가하므로(단일 에이전트 ~4배, 멀티 에이전트 ~15배), 고가치 단계에만 최고 티어를 투입한다. **티어 완화는 비용 근거로만 결정하고, 품질 영향은 가정하지 않는다** — 하위 티어로 내릴 단계는 Phase 6-3의 With/Without 비교로 도메인별 검증한 뒤 적용한다. 워크플로우 모드에서는 스테이지별 모델 라우팅과 토큰 예산(`budget`)으로 이를 네이티브로 표현한다(`references/workflow-mode.md` 6장).
...
- **워크플로우**: 번들 스크립트의 `meta` 블록이 순수 리터럴인지, `phase()` 호출과 `meta.phases` title이 일치하는지, `parallel`/`pipeline` 선택이 배리어 필요성과 맞는지, 각 `agent()`가 `_workspace/` 기록 지시를 포함하는지, Workflow 도구 불가 시 폴백 경로가 명시됐는지 확인 (검증 기준은 `references/workflow-mode.md` 8장)
...
- [ ] 실행 모드 명시 (에이전트 팀 / 서브 에이전트 / 하이브리드 / 워크플로우 중 선택, 하이브리드면 Phase별 모드 기재, 워크플로우면 폴백 경로 명시)
- [ ] Agent 호출 모델 티어 설정 — 핵심 단계는 `model: "opus"`, 단순·기계적 단계는 하위 티어 허용(비용 근거, 도메인별 A/B로 검증)
...
- **워크플로우 모드 가이드**: `references/workflow-mode.md` — Claude Code Workflow 도구 기반 4번째 실행 모드. 프리미티브, 6패턴↔프리미티브 매핑, Template D, 모델 티어링·예산, 재개. **research preview**이므로 버전 게이팅 필요.
