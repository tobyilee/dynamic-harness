...
### 3-5. 워크플로우 모드 매핑: 교차검증 = 병렬 차원, incremental QA = pipeline

§2의 4개 교차검증(API↔훅 / 경로↔href / 상태전이 / 엔드포인트↔훅)은 서로 **독립된 검증 차원**이다 → 워크플로우 모드에서 `parallel`로 동시 실행한다. §3-4의 incremental QA는 `pipeline`의 **배리어 없음** 의미와 정확히 일치한다: 모듈 A를 QA하는 동안 모듈 B 빌드가 진행된다.

```js
// incremental QA — 모듈별 빌드 직후 QA (배리어 없음), 각 모듈은 4개 차원을 동시 검증
const qa = await pipeline(modules,
  m => agent(`${m} 구현`, { label: `build:${m}` }),
  (built, m) => parallel(DIMENSIONS.map(d => () =>
    agent(`${m}의 ${d.name} 정합성 검증 — 생산자·소비자 양쪽 동시 읽기`, { label: `qa:${m}:${d.key}`, schema: QA_VERDICT }))))
```

"양쪽 동시 읽기"(§3-3)는 어드버서리얼 교차검증의 본질이며, 각 차원을 독립 에이전트가 맡으면 한 차원의 누락이 다른 차원을 오염시키지 않는다. 상세는 `references/workflow-mode.md`.
